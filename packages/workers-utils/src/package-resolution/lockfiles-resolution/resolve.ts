 ...
 * Keyed by absolute lockfile path. Lockfiles are not expected to change
 * during a single process invocation (e.g. a `wrangler deploy`), so a
 * simple path-keyed cache is sufficient.
 */
 ...
 * path, so repeated per-package lookups within a single process (e.g.
 * inside `collectPackageDependencies`) are O(1) after the initial parse.
 *
...
export function getInstalledVersionsFromLockfile(
	...
	if (useCache) {
		...
		if (versions) {
			cache.set(lockfilePath, versions);
		}
		return versions;
	}
